...
def select_project():
    ...
    sql="select project_name from npm_all_project_name where dependencies_num!=0 and dependencies_num is not null " \
         "and dependencies_num>1000 and dependencies_num<5000 and reduce_duplicate_size_proportion<1 and reduce_duplicate_count_proportion<1 " \
         "and reduce_count<duplicate_dependencies_num and duplicate_dependencies_num!=0 " \
         "and project_name in (select project_name from npm_all_releases where  repository='github' " \
         "group by project_name) and project_name not in (select project_name from npm_statistical_data_duplicate) ORDER BY dependencies_num DESC limit 2000"

    # sql = "select project_name from npm_all_project_name where dependencies_num!=0 and dependencies_num is not null " \
    #       "and dependencies_num>1000 and dependencies_num<5000 and reduce_duplicate_size_proportion<1 and reduce_duplicate_count_proportion<1 " \
    #       "and reduce_count<duplicate_dependencies_num and duplicate_dependencies_num!=0 " \
    #       "and project_name in (select project_name from npm_all_releases where  repository='github' " \
    #       "group by project_name) ORDER BY dependencies_num DESC limit 2000"
    try:
...
def latest_version(project_name):
    name_list = []
    # SQL 查询语句
    sql = "SELECT package_version,unpacked_size FROM npm_all_releases where project_name='%s' ORDER BY creat_time DESC limit 1" % (
        project_name)
    try:
...
def get_version_list(project_name):
    version_list = []
    # SQL 查询语句
    sql = "SELECT package_version FROM npm_all_releases where project_name='%s' ORDER BY creat_time DESC" % (
        project_name)
    try:
        ...
        print("get_version_list()  dberror", e, project_name)
    for data in datas:
...
def get_dependencies(project_name, package_version):
    ...
    sql = "SELECT dependency,version_range FROM " + table_name + " where project_name='%s' and package_version='%s' ORDER BY priority ASC" % (
...
def get_unpackedsize(projectname, projectversion):
    # SQL 查询语句
    temp_size = 0
    sql = "SELECT unpacked_size FROM npm_all_releases where project_name='%s' and package_version='%s' limit 1" % (
    ...
    for dependency in dependencies_list:
                ...
                for temp_version in version_list:
                    if check_version_range(temp_version, version_range):
                        dep_obj = {"dep_name": dependencyname, "dep_version": temp_version,
                                   "dep_version_range": version_range, "direct_dependency": direct_dependency}
        ...
        # top层有同名但版本范围不兼容
            ...
            for dp_temp_sandbox in tp_dependencies_list:
                ...
                if dependencyname == dp_temp_sandbox_name and check_version_range(dp_temp_sandbox_verison,
                                                                                  version_range):
                    # sandbox有同名且版本范围兼容
                    dp_tp_status = 1
...
def single_deduplicate(projectname):
    try:
        ...
        reduce_dependencies_list, dependencies_num = do_deduplicate(dependencies_list, unpacked_size, 0, 0,projectname)
        ...
        for i in range(100):
            print("***********************************************************")
            # 取reduce_dependencies_list重复次数最多的依赖项
            max_num = 0
            max_deduplicatenum = 0
            for di in range(len(temp_reduce_dependencies_list)):
                if int(temp_reduce_dependencies_list[di][2]) > max_deduplicatenum:
                    dp_status = 0
                    for dp in dependencies_list:
                        if temp_reduce_dependencies_list[di][0] == dp[0]:
                            dp_status = 1
                    if dp_status == 0:
                        max_deduplicatenum = temp_reduce_dependencies_list[di][2]
                        max_num = di

            reduce_dependency = []
            reduce_dependency.append(temp_reduce_dependencies_list[max_num][0])
            reduce_dependency.append(temp_reduce_dependencies_list[max_num][1])

            # print(reduce_dependency, max_deduplicatenum)
            temp_add_dependencies_list.append(reduce_dependency)
            dependencies_list.append(reduce_dependency)
            temp_reduce_dependencies_list = []
            TOP_dependencies_list = []
            OTHER_dependencies_list = []
            temp_reduce_dependencies_list, dependencies_num = do_deduplicate(dependencies_list, unpacked_size,
                                                                             total_depnum, i,projectname)
        ...
        #     # f.write("#####################################" + "\n")
        #     # for dii in dependencies_list:
        #     #     f.write("\"" + str(dii[0]) + "\"" + ": " + "\"" + str(dii[1]) + "\"" + "," "\n")
    except Exception as e:
        print("test error ", e)
        pass


def do_deduplicate(dependencies_list, unpacked_size, total_depnum, i,projectname):
    ...
    # insert_statistical_data(projectname, i, total_depnum, dependencies_num,
    #                         duplicate_dependencies_num, duplicate_dependencies_proportion,
    #                         total_depnum - dependencies_num, total_reduce_count, reduce_duplicate_count_proportion)
    with open(url, 'a', encoding='utf-8') as f:
        f.write("********************************" + "\n")
        f.write("第" + str(i) + "次迭代" + "\n")
    ...
    #         temp_other_versionrange = temp_otherinfo["dep_version_range"]
    #         temp_other_directdependency = temp_otherinfo["direct_dependency"]
    #         f.write(str(temp_other_name) + str(temp_other_versionrange) + "\n")
    #     f.write("~~~~~~~~~~~~~~~~~~~~~~~~~~~~" + "\n")
...
def insert_statistical_data(project_name,dedup_times,total_dependencies_num,dependencies_num,duplicate_dependencies_num,duplicate_dependencies_proportion,
    ...
    sql_insert = "INSERT INTO npm_statistical_data_duplicate (project_name,dedup_times,total_dependencies_num,dependencies_num,duplicate_dependencies_num,duplicate_dependencies_proportion," \
                 "reduce_duplicate_depnum,reduce_count,reduce_duplicate_count_proportion) " \
...
def readJSON():
    with open("D:/nodejs_test/react-loaders-kit_new/package-lock.json", 'r') as load_f:
        ...
        for dependency in dependencies:
            print(dependency, dependencies[dependency]['version'])
        ...
        do_deduplicate(dependencies_list, 0, 0, 0)
...
if __name__ == '__main__':
    ...
    url = "D://nodejsinfo//nodejsinfo20200615-04.txt"
    # main()
    single_deduplicate("@eshlox/gatsby-theme-axii")
    # readJSON()
